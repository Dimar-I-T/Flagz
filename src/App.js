...
function Begin() {
  ...
  useEffect(() => {
    axios.get("https://flagz-seven.vercel.app/api").then(result => {
...
function Home() {
  ...
  useEffect(() => {
    ...
    axios.get("https://flagz-seven.vercel.app/api/home").then(result => {
      setData(result.data);
    })

    axios.get("https://flagz-seven.vercel.app/api/f").then(result => {
  ...
  const stopPlaying = () => {
    axios.post("https://flagz-seven.vercel.app/api/home", {score: score});
    setBenar(2);
    setStop(1);
  }
  ...
  const handleSubmit = (e) => {
    ...
    setBanyak(banyak + 1);
    if (banyak < 196){
      axios.post("https://flagz-seven.vercel.app/api/cek", {answer, dataF: dataF[indeks].country}).then((response) => {
        if (response.data == "1"){
          setBenar(1);
          setBBenar(bBenar + 1);
          setScore(score + 50);
...
function Reg() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post("https://flagz-seven.vercel.app/api/register", {valUser, valPass}).then((response) => {
...
function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post("https://flagz-seven.vercel.app/api/login", {valUser, valPass}).then((response) => {
      const resp = JSON.stringify(response.data.success, null, 2)
      if (resp == "true"){
        setBisa(1);
        navigate('/home');
